...
namespace RE::FO4Runtime
{
	...
	namespace PreNG
	{
		// TODO(PostAE LLF): This is the verified PreNG 1.10.163 layout.
		// Keep it for PreNG source compatibility only; do not use these raw
		// addresses or field offsets as PostAE LLF runtime data.
		...
		inline constexpr std::uint32_t DF_COMPOSITE_SHADER_TYPE = 6;  // TODO(PostAE): verify shader contract before enabling LLF consumer.
		inline constexpr std::uint32_t DF_LIGHT_FULL_CONTRACT_PIXEL_DESCRIPTOR_20201 = 0x00020201;
		inline constexpr std::uint32_t DF_LIGHT_COMPACT_SHADOWED_PIXEL_DESCRIPTOR_200208 = 0x00200208;  // TODO(PostAE): verify before enabling LLF consumer.
		...
		{
			// TODO(PostAE LLF): Keep the full-shadowed proof descriptors out of
		...
		{
			// TODO(PostAE LLF): Verify these against PostAE runtime dumps before use.
	...
	namespace PostNG
	{
		// TODO(PostAE LLF): PostAE builds currently share this PostNG namespace.
		// Add audited PostAE runtime data for BSRenderPass light tables,
		// BSShadowLight maskIndex, ShadowSceneNode active buckets,
